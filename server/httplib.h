  ...
  setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<void *>(&yes),
	...
	inline std::string to_string(const Error error)
	{
		switch (error) {
			...
			case Error::UnsupportedMultipartBoundaryChars: return
					"UnsupportedMultipartBoundaryChars";
...
private:
	...
	namespace detail {
		                          ...
		                          int& val)
		{
			...
			for (; cnt; i++, cnt--) {
				if (!s[i]) { return false; }
				int v = 0;
				if (is_hex(s[i], v)) { val = val * 16 + v; } else { return false; }
			}
			return true;
		}
  ...
  return stat(path.c_str(), &st) >= 0 && S_ISREG(st.st_mode);
		...
		inline bool is_valid_path(const std::string& path)
		{
			...
			while (i < path.size()) {
				...
				if (!path.compare(beg, len, ".")) { ; } else if (!path.
					compare(beg, len, "..")) {
		...
		inline std::string encode_url(const std::string& s)
		{
			...
			for (size_t i = 0; s[i]; i++) {
				switch (s[i]) {
					...
					case ';': result += "%3B";
						break;
					default: auto c = static_cast<uint8_t>(s[i]);
						if (c >= 0x80) {
							result += '%';
							char hex[4];
		                              ...
		                              bool convert_plus_to_space)
		{
			...
			for (size_t i = 0; i < s.size(); i++) {
				...
				} else if (convert_plus_to_space && s[i] == '+') { result += ' '; } else {
					result += s[i];
				}
			}
		...
		{
			if (glowable_buffer_.empty()) { return fixed_buffer_; } else {
		...
		{
			if (glowable_buffer_.empty()) { return fixed_buffer_used_size_; } else {
		...
		inline bool stream_line_reader::getline()
		{
			...
			for (size_t i = 0;; i++) {
				...
				if (n < 0) { return false; } else if (n == 0) {
  ...
  if (sock >= FD_SETSIZE) { return Error::Connection; }
		...
		inline bool is_socket_alive(socket_t sock)
		{
			...
			if (val == 0) { return true; } else if (val < 0 && errno == EBADF) {
				return false;
			}
			char buf[1];
  ...
  time_t write_timeout_usec_;
		...
		inline bool keep_alive(socket_t sock, time_t keep_alive_timeout_sec)
		{
			...
			while (true) {
				...
				if (val < 0) { return false; } else if (val == 0) {
  ...
  return addr_candidate;
			...
			time_t write_timeout_usec, const std::string& intf, Error& error)
		{
          ...
          setsockopt(sock2, SOL_SOCKET, SO_SNDTIMEO, (char *)&tv, sizeof(tv));
			...
			if (sock != INVALID_SOCKET) { error = Error::Success; } else {
		...
		inline void get_local_ip_and_port(socket_t sock, std::string& ip, int& port)
		{
			...
			if (!getsockname(sock, reinterpret_cast<struct sockaddr*>(&addr),
			                 &addr_len)) { get_ip_and_port(addr, addr_len, ip, port); }
		}
      ...
      return;
		...
		inline bool can_compress_content_type(const std::string& content_type)
		{
			...
			switch (tag) {
				...
				default: return !content_type.rfind("text/", 0) && tag !=
      ...
      line_terminator_len = 1;
		                                     ...
		                                     ContentReceiverWithProgress out)
		{
			...
			while (r < len) {
				...
				if (progress) { if (!progress(r, len)) { return false; } }
			}
		                                        ...
		                                        ContentReceiverWithProgress out)
		{
			...
			for (;;) {
				...
				if (n < 0) { return false; } else if (n == 0) { return true; }
      ...
      decompressor = detail::make_unique<brotli_decompressor>();
		                          ...
		                          Error& error)
		{
			...
			data_sink.write = [&](const char* d, size_t l) -> bool {
				if (ok) {
					if (write_data(strm, d, l)) { offset += l; } else { ok = false; }
				}
				return ok;
			};
		...
		inline void parse_query_text(const std::string& s, Params& params)
		{
			...
			split(s.data(), s.data() + s.size(), '&', [&](const char* b, const char* e) {
				...
				split(b, e, '=', [&](const char* b2, const char* e2) {
					if (key.empty()) { key.assign(b2, e2); } else {
						val.assign(b2, e2);
					}
				});
...
inline bool parse_range_header(const std::string &s, Ranges &ranges) {
		...
		inline bool expect_content(const Request& req)
		{
			if (req.method == "POST" || req.method == "PUT" || req.method == "PATCH" ||
				req.method == "PRI" || req.method == "DELETE") { return true; }
...
inline std::pair<std::string, std::string> make_digest_authentication_header(
  ...
  return std::make_pair(key, field);
		                                   ...
		                                   bool is_proxy)
		{
			...
			if (res.has_header(auth_key)) {
				...
				if (pos != std::string::npos) {
					...
					if (type == "Basic") { return false; } else if (type == "Digest") {
						s = s.substr(pos + 1);
	...
	inline void Response::set_redirect(const std::string& url, int stat)
	{
		if (!detail::has_crlf(url)) {
			...
			if (300 <= stat && stat < 400) { this->status = stat; } else {
				this->status = 302;
			}
		}
	}
  ...
  size = (std::min)(size,
			...
			if (size < read_buff_size_) {
				...
				if (n <= 0) { return n; } else if (n <= static_cast<ssize_t>(size)) {
			...
			} else { return read_socket(sock_, ptr, size, CPPHTTPLIB_RECV_FLAGS); }
		}
  ...
  signal(SIGPIPE, SIG_IGN);
	...
	inline bool Server::parse_request_line(const char* s, Request& req)
	{
...

		{
			...
			detail::split(s, s + len, ' ', [&](const char* b, const char* e) {
				switch (count) {
					case 0: req.method = std::string(b, e);
						break;
					case 1: req.target = std::string(b, e);
						break;
					case 2: req.version = std::string(b, e);
						break;
					default: break;
				}
				count++;
...

		{
			...
			detail::split(req.target.data(), req.target.data() + req.target.size(), '?',
			              [&](const char* b, const char* e) {
				              switch (count) {
					              case 0: req.path = detail::decode_url(
							              std::string(b, e), false);
						              break;
					              case 1: {
						              if (e - b > 0) {
...
#endif
				} else { compressor = detail::make_unique<detail::nocompressor>(); }
	...
	inline bool Server::routing(Request& req, Response& res, Stream& strm)
	{
		...
		if ((req.method == "GET" || is_head_request) &&
			handle_file_request(req, res, is_head_request)) { return true; }
			...
			// Content reader handler
			{
				...
				if (req.method == "POST") {
					if (dispatch_request_for_content_reader(
						...
						post_handlers_for_content_reader_)) { return true; }
				} else if (req.method == "PUT") {
					if (dispatch_request_for_content_reader(
						req, res, std::move(reader),
						put_handlers_for_content_reader_)) { return true; }
				} else if (req.method == "PATCH") {
					if (dispatch_request_for_content_reader(
						req, res, std::move(reader),
						patch_handlers_for_content_reader_)) { return true; }
				} else if (req.method == "DELETE") {
					if (dispatch_request_for_content_reader(
						req, res, std::move(reader),
						delete_handlers_for_content_reader_)) { return true; }
				}
			}
		...
		if (req.method == "GET" || req.method ==
			"HEAD") { return dispatch_request(req, res, get_handlers_); } else if (
			req.method ==
			"POST") { return dispatch_request(req, res, post_handlers_); } else if (
			req.method ==
			"PUT") { return dispatch_request(req, res, put_handlers_); } else if (
			req.method ==
			"DELETE") { return dispatch_request(req, res, delete_handlers_); } else if (
			req.method == "OPTIONS") {
	                                 ...
	                                 std::string& boundary)
	{
		...
		if (res.body.empty()) {
			if (res.content_length_ > 0) {
				size_t length = 0;
				if (req.ranges.empty()) { length = res.content_length_; } else if (
					req.ranges.size() == 1) {
					auto offsets =
						detail::get_range_offset_and_length(req, res.content_length_, 0);
					auto offset = offsets.first;
...
#else
		try { routed = routing(req, res, strm); }
  ...
  const static std::regex re("(HTTP/1\\.[01]) (\\d{3})(?: (.*?))?\r\n");
	...
	inline bool ClientImpl::send(Request& req, Response& res, Error& error)
	{
      ...
      if (is_ssl()) {
		...
		if (!ret) { if (error == Error::Success) { error = Error::Unknown; } }
	                                       ...
	                                       Error& error)
	{
		...
		} else { ret = process_request(strm, req, res, close_connection, error); }
  ...
  if ((res.status == 401 || res.status == 407) &&
	...
	inline bool ClientImpl::redirect(Request& req, Response& res, Error& error)
	{
		...
		if (!port_str.empty()) { next_port = std::stoi(port_str); } else if (!
			next_scheme.empty()) { next_port = next_scheme == "https" ? 443 : 80; }
	                                      ...
	                                      bool close_connection, Error& error)
	{
		...
		if (!req.has_header("Host")) {
			if (is_ssl()) {
				if (port_ == 443) { req.headers.emplace("Host", host_); } else {
					req.headers.emplace("Host", host_and_port_);
				}
			} else {
				if (port_ == 80) { req.headers.emplace("Host", host_); } else {
					req.headers.emplace("Host", host_and_port_);
				}
			}
		}
		...
		if (req.body.empty()) {
			if (req.content_provider_) {
				...
				if (req.method == "POST" || req.method == "PUT" ||
					req.method == "PATCH") { req.headers.emplace("Content-Length", "0"); }
			}
		} else {
    ...
    if (content_provider) {
      ...
      if (!compressor.compress(body, content_length, true,
...
#endif
		{
			...
			} else { req.body.assign(body, content_length);; }
		}
